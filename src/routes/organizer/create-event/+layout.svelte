<script lang="ts">
  ...
  const API_BASE_URL = (envUrl || "https://158.108.102.14:8005").replace(
    /\/$/,
    "",
  );
  ...
  const translations = {
    th: {
      ...
      excludeWeekendsOption: "หยุดเสาร์-อาทิตย์",
      ...
      rewardHelperText:
        "💡 ระบบจะจัดอันดับตามความยาก ผู้ที่วิ่งได้มากกว่าจะได้รางวัลที่ดีกว่า (จำกัด {total} คนแรก)",
      tierHelperText:
        "ผู้เข้าร่วมที่วิ่งครบจำนวนนี้จะได้รับรางวัล (ถ้าอยู่ใน {total} คนแรก)",
      ...
      addSpecificDateBtn: "เพิ่มวันหยุด",
      selectHolidayPlaceholder: "เลือกวันที่หยุดจากช่วงเวลากิจกรรม",
    ...
    en: {
      ...
      rewardHelperText:
        "💡 System ranks by difficulty. Top performers get better rewards (limited to top {total})",
      tierHelperText:
        "Participants who complete this many runs will receive this reward (if in top {total})",
      ...
      addSpecificDateBtn: "Add Holiday",
      selectHolidayPlaceholder: "Select holiday within event range",
  ...
  let formData = {
    ...
    rewardTiers: [
      { reward_name: "", required_completions: null },
    ] as RewardTierConfig[],
    isPublic: true,
    isActive: true,
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          Array.isArray(holidaysRes.data.holidays) &&
          holidaysRes.data.holidays.length > 0
        ) {
      ...
      // loaded status
      formData.imagePreview = resolveImageUrl(
        data.banner_image_url || data.image || data.image_url,
      );
      ...
      try {
        ...
        if (rewardApiAvailable) {
          try {
            const configRes = await api.get(
              `/api/reward-leaderboards/configs/event/${id}`,
            );
            ...
            if (config) {
              ...
              if (config.reward_tiers && config.reward_tiers.length > 0) {
                formData.rewardTiers = config.reward_tiers.map((t: any) => {
                  const rewardName =
                    t.reward_name || t.reward?.name || t.name || "";
                  return {
            ...
            if (err.response?.status === 404) {
              console.warn(
                "Reward leaderboard API not present; skipping config fetch.",
              );
              rewardApiAvailable = false;
            } else if (err.response?.status === 500) {
              console.warn(
                "Reward config exists but has errors - will recreate on save",
              );
        ...
        console.warn(
          "Skipping reward config due to availability check error",
          outerErr,
        );
        editingRewardConfigId = null;
      }
  ...
  function calculateMaxCheckins() {
    ...
    try {
      ...
      console.log(
        `📊 Max check-ins: ${totalDays} days - ${holidayCount} holidays = ${availableDays}`,
      );
    } catch (e) {
  ...
  function handlePreviewImgError(e: Event) {
    ...
    if (img)
      img.src =
        "https://placehold.co/600x400/1e293b/64748b?text=Image+Not+Found";
  }
  ...
  function removeRewardTier(index: number) {
    if (formData.rewardTiers.length > 1) {
      formData.rewardTiers = formData.rewardTiers.filter((_, i) => i !== index);
    }
  }
  ...
  function validate(): boolean {
    ...
    if (!formData.title) {
      ...
      errorMessages.push("• กรุณากรอกชื่อกิจกรรม");
    } else if (formData.title.length < 3) {
      validationErrors.add("title");
      errorMessages.push("• ชื่อกิจกรรมต้องมีอย่างน้อย 3 ตัวอักษร");
    }

    if (!formData.description) {
      validationErrors.add("description");
      errorMessages.push("• กรุณากรอกรายละเอียดกิจกรรม");
    }

    if (!formData.location) {
      validationErrors.add("location");
      errorMessages.push("• กรุณากรอกสถานที่จัดกิจกรรม");
    }

    // ✅ Date validation
    if (!formData.sDay || !formData.sMonth || !formData.sYear) {
      validationErrors.add("startDate");
      errorMessages.push("• กรุณาเลือกวันที่เริ่มต้น");
    }

    if (!formData.eDay || !formData.eMonth || !formData.eYear) {
      validationErrors.add("endDate");
      errorMessages.push("• กรุณาเลือกวันที่สิ้นสุด");
    }

    // ✅ Time validation
    if (!formData.startTime) {
      validationErrors.add("startTime");
      errorMessages.push("• กรุณาเลือกเวลาเริ่มต้น");
    }

    if (!formData.endTime) {
      validationErrors.add("endTime");
      errorMessages.push("• กรุณาเลือกเวลาสิ้นสุด");
    }

    // ✅ Capacity validation
    if (!formData.totalSlots) {
      validationErrors.add("totalSlots");
      errorMessages.push("• กรุณากรอกจำนวนที่นั่ง");
    } else if (Number(formData.totalSlots) <= 0) {
      validationErrors.add("totalSlots");
      errorMessages.push("• จำนวนที่นั่งต้องมากกว่า 0");
    }

    // ✅ ตรวจสอบว่าวันสิ้นสุดต้องอยู่หลังวันเริ่มต้น
    if (
      formData.sDay &&
      formData.sMonth &&
      formData.sYear &&
      ...
      const sMonthIdx = months.indexOf(formData.sMonth) + 1;
      const eMonthIdx = months.indexOf(formData.eMonth) + 1;
      const startDateStr = `${formData.sYear}-${String(sMonthIdx).padStart(2, "0")}-${String(formData.sDay).padStart(2, "0")}T${formData.startTime}:00`;
      const endDateStr = `${formData.eYear}-${String(eMonthIdx).padStart(2, "0")}-${String(formData.eDay).padStart(2, "0")}T${formData.endTime}:00`;
      const startDate = new Date(startDateStr);
      const endDate = new Date(endDateStr);

      // Format dates for display
      const startDisplay = `${formData.sDay}.${formData.sMonth}.${formData.sYear} เวลา ${formData.startTime}`;
      const endDisplay = `${formData.eDay}.${formData.eMonth}.${formData.eYear} เวลา ${formData.endTime}`;

      if (endDate <= startDate) {
        validationErrors.add("endDate");
        validationErrors.add("startDate");
        errorMessages.push(`<b style="color:#ef4444;">วันที่ไม่ถูกต้อง</b>`);
        errorMessages.push(`วันเริ่มต้น: <b>${startDisplay}</b>`);
        errorMessages.push(`วันสิ้นสุด: <b>${endDisplay}</b>`);
        errorMessages.push(
          `<span style="color:#f59e0b;">วันสิ้นสุดต้องอยู่หลังวันเริ่มต้น</span>`,
        );
      }
    ...
    if (formData.totalRewards && formData.rewardTiers.length > 0) {
      const validTiers = formData.rewardTiers.filter(
        (t) => t.reward_name && t.required_completions,
      );

      if (Number(formData.totalRewards) < validTiers.length) {
        validationErrors.add("rewards");
        errorMessages.push(
          `จำนวนรางวัลทั้งหมด (${formData.totalRewards}) น้อยกว่าจำนวนระดับรางวัล (${validTiers.length})`,
        );
      }

      // ตรวจ required_completions ต้อง > 0
      const invalidTiers = formData.rewardTiers.filter((t, idx) => {
        if (!t.reward_name) return false; // skip empty tiers
        return !t.required_completions || Number(t.required_completions) <= 0;
      });

      if (invalidTiers.length > 0) {
        validationErrors.add("rewards");
        errorMessages.push(
          "ระดับรางวัลทุกตัวต้องกำหนดจำนวนรอบที่ต้องวิ่ง (> 0)",
        );
      }
    }

    // ✅ Show combined error message if any
    if (errorMessages.length > 0) {
      Swal.fire({
        icon: "warning",
        title: "กรุณาตรวจสอบข้อมูล",
        html: `<div style="text-align:center;line-height:2;">${errorMessages.join("<br>")}</div>`,
        customClass: { popup: "swal-dark" },
        confirmButtonColor: "#10b981",
      });
    }
  ...
  async function submit() {
    ...
    // Validate Inputs - validate() already shows specific error messages via Swal
    if (!validate()) {
      isSubmitting = false;
      return;
    }
    ...
    if (!token) {
      Swal.fire({
        title: "Auth Error",
        text: "ไม่พบข้อมูลการเข้าสู่ระบบ กรุณา Login ใหม่",
        icon: "error",
      });
      goto("/auth/login");
      return;
    ...
    try {
      ...
      const startDateISO = new Date(
        `${startDateStr}T${formData.startTime}:00`,
      ).toISOString();
      const endDateISO = new Date(
        `${endDateStr}T${formData.endTime}:00`,
      ).toISOString();
      ...
      if (formData.eventType === "multi_day" && targetId) {
        ...
        if (formData.holidayType === "weekends") {
          ...
          formData.holidayType === "public" ||
          formData.holidayType === "weekends_public"
        ) {
          const publicHols = holidaysData.filter(
            (h: any) => h.date >= startDateStr && h.date <= endDateStr,
          );
      ...
      if (
        ...
        formData.rewardTiers[0].reward_name &&
        targetId
      ) {
        // Safety Check
        ...
        if (total < formData.rewardTiers.length) {
          throw new Error(
            `จำนวนรางวัลทั้งหมด (${total}) น้อยกว่าจำนวนระดับรางวัล (${formData.rewardTiers.length}) กรุณาเพิ่มจำนวนรางวัล`,
          );
        }
        ...
        const sortedTiers = [...formData.rewardTiers].sort((a, b) => {
          return (
            (Number(b.required_completions) || 0) -
            (Number(a.required_completions) || 0)
          );
        });

        const createdTiers = await Promise.all(
          sortedTiers.map(async (t, idx) => {
            if (
              !t.reward_name ||
              !t.required_completions ||
              t.required_completions <= 0
            )
              return null;
            ...
            try {
              const timePeriodDays =
                (t.time_period_days && Number(t.time_period_days)) || 30;
        ...
        if (validTiers.length > 0) {
          const rewardConfigPayload = {
            ...
            max_reward_recipients: Math.max(1, total),
            ...
            is_active: true, // ✅ เพิ่ม field ที่ Backend ต้องการ
            ...
            reward_tiers: validTiers.map((t, idx) => {
              ...
              return {
                ...
                min_rank: Math.max(1, rankStart),
                max_rank: Math.max(1, rankStart + quantity - 1),
                reward_id: tierObj.reward_id,
                reward_name: tierObj.reward_name,
                quantity: Math.max(1, quantity),
                required_completions: Math.max(
                  1,
                  Number(tierObj.required_completions || 1),
                ),
              };
            }),
          };
          ...
          try {
            ...
            try {
              const checkRes = await api.get(
                `/api/reward-leaderboards/configs/event/${targetId}`,
              );
              const configData = Array.isArray(checkRes.data)
                ? checkRes.data[0]
                : checkRes.data;
            ...
            if (realConfigId) {
              try {
                const updRes = await api.put(
                  `/api/reward-leaderboards/configs/${realConfigId}`,
                  rewardConfigPayload,
                );
                ...
                if (isFinalizedError) {
                  ...
                  try {
                    ...
                    editingRewardConfigId =
                      createRes.data?.id || createRes.data?.config_id;
              ...
              try {
                console.log(
                  "📦 [DEBUG] Creating config with payload:",
                  JSON.stringify(rewardConfigPayload, null, 2),
                );
                const createRes = await api.post(
                  "/api/reward-leaderboards/configs",
                  rewardConfigPayload,
                ...
                // แสดง validation errors จาก Pydantic
                if (
                  createErr.response?.status === 422 &&
                  createErr.response?.data?.detail
                ) {
                  const details = createErr.response.data.detail;
                  console.error(
                    "🔍 Validation errors:",
                  ...
                  if (Array.isArray(details)) {
                    errorMessages = details
                      .map((d: any) => {
                        const loc = d.loc?.join(" -> ") || "unknown field";
                        return `• ${loc}: ${d.msg}`;
                      })
                      .join("\n");
                  } else {
                  ...
                  Swal.fire({
                    ...
                    html: `<pre style="text-align:left;background:#1e293b;padding:12px;border-radius:8px;overflow:auto;max-height:300px;font-size:12px;">${errorMessages}</pre>`,
                    customClass: { popup: "swal-dark" },
                  });
                }
            ...
            let msg = rewErr.response?.data?.detail
              ? typeof rewErr.response.data.detail === "object"
                ? JSON.stringify(rewErr.response.data.detail)
                : rewErr.response.data.detail
              : rewErr.message;
  ...
  <div class="ce-container">
      ...
      <div class="ce-header-right">
        ...
        <div class="ce-header-actions">
          <button
            class="ce-btn-cancel"
            on:click={cancel}
            disabled={isSubmitting}>{lang.cancel}</button
          >
          <button
            class="ce-btn-save"
            on:click={submit}
          ...
          >
            ...
            {:else}
              <span class="btn-label"
                >{editingEventId ? lang.update : lang.publish}</span
              >
            {/if}
          </button>
        </div>
    ...
    <div class="ce-grid-layout">
      <div
        ...
        role="button"
        tabindex="0"
        on:keydown|stopPropagation={handleImgCardKeydown}
      >
        <input
          type="file"
          accept="image/*"
      ...
      <div class="ce-card ce-form-card">
        ...
        <div class="ce-input-group">
          ...
          ><input
            ...
            class="ce-input"
            class:error={validationErrors.has("title")}
          />
        </div>
        <div class="ce-input-group">
          ...
          ><textarea
            ...
            class="ce-textarea"
            class:error={validationErrors.has("description")}
          ></textarea>
        </div>
        <div class="ce-input-group">
          <label for="location"
          ...
          ><input
            ...
            placeholder="สถานที่"
            class="ce-input"
            class:error={validationErrors.has("location")}
          />
        </div>
      </div>
      ...
      <div class="ce-card ce-config-card">
        ...
        <div class="ce-event-type-buttons">
          <button
            ...
            class:active={formData.eventType === "single_day"}
            on:click={() => setEventType("single_day")}
          >
            ...
            <div class="ce-event-type-content">
              <span class="ce-event-type-title">{lang.singleDay}</span><span
                class="ce-event-type-desc">{lang.singleDayDesc}</span
              >
            </div>
            {#if formData.eventType === "single_day"}<div
                class="ce-event-type-check"
              >
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"
                  ><path
                    stroke-linecap="round"
          ...
          <button
            type="button"
            class="ce-event-type-btn"
            class:active={formData.eventType === "multi_day"}
            on:click={() => setEventType("multi_day")}
          >
            <div class="ce-event-type-icon">
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"
                ><path
                ...
                ></path><path
                  ...
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="M7 13h2m2 0h2m2 0h2M7 16h2m2 0h2m2 0h2"
                  stroke="currentColor"
                  opacity="0.6"
                /></svg
              >
            ...
            <div class="ce-event-type-content">
              <span class="ce-event-type-title">{lang.multiDay}</span><span
                class="ce-event-type-desc">{lang.multiDayDesc}</span
              >
            </div>
            {#if formData.eventType === "multi_day"}<div
                class="ce-event-type-check"
              >
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"
                  ><path
        ...
        {#if formData.eventType === "multi_day"}
          <div class="ce-checkin-section">
            ...
            <div class="ce-checkin-stepper">
              <button
                type="button"
                class="ce-stepper-btn"
                aria-label="Decrease max check-ins"
                on:click={() => {
                  if (formData.maxCheckinsPerUser > 1)
                    formData.maxCheckinsPerUser--;
                }}
                disabled={formData.maxCheckinsPerUser <= 1}
              ...
              <button
                type="button"
                class="ce-stepper-btn"
                aria-label="Increase max check-ins"
                on:click={() => {
                  if (formData.maxCheckinsPerUser < 365)
                    formData.maxCheckinsPerUser++;
                }}
                disabled={formData.maxCheckinsPerUser >= 365}
                ><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
                  ><path
                    stroke-linecap="round"
                    stroke-linejoin="round"
      ...
      <div class="ce-card ce-config-card">
        ...
        <div class="ce-input-group">
          ...
          <div
            class="ce-date-row"
            class:error={validationErrors.has("startDate")}
          >
            <div class="ce-dd-wrap flex-1-5">
              <div
                ...
                role="button"
                tabindex="0"
                on:click|stopPropagation={() => toggleDropdown("sDay")}
                on:keydown|stopPropagation={(e) =>
                  handleTriggerKeydown(e, "sDay")}
              >
                <input
                  id="sDayInput"
                  type="text"
                  value={formData.sDay}
                  placeholder={lang.dayPlaceholder}
                  class="ce-input-dis"
                  readonly
                /><span class="ce-arrow">▼</span>
              </div>
              {#if activeDropdown === "sDay"}<div
                  class="ce-options"
                  role="listbox"
                  tabindex="0"
                ...
                >
                  {#each days as d}<button
                      class="ce-opt"
                      on:click|stopPropagation={() => selectOption("sDay", d)}
                      >{d}</button
                    >{/each}
                </div>{/if}
            </div>
            <div class="ce-dd-wrap flex-2">
              <div
                ...
                role="button"
                tabindex="0"
                on:click|stopPropagation={() => toggleDropdown("sMonth")}
                on:keydown|stopPropagation={(e) =>
                  handleTriggerKeydown(e, "sMonth")}
              >
                <input
                  id="sMonthInput"
                  type="text"
                  value={translateMonth(formData.sMonth)}
                  placeholder={lang.monthPlaceholder}
                  class="ce-input-dis"
                  readonly
                /><span class="ce-arrow">▼</span>
              </div>
              {#if activeDropdown === "sMonth"}<div
                  class="ce-options"
                  role="listbox"
                  tabindex="0"
                  on:click|stopPropagation
                ...
                >
                  {#each months as m, idx}<button
                      class="ce-opt"
                      on:click|stopPropagation={() => selectOption("sMonth", m)}
                      >{displayMonths[idx]}</button
                    >{/each}
                </div>{/if}
            </div>
            <div class="ce-dd-wrap flex-1-5">
              <div
                ...
                role="button"
                tabindex="0"
                on:click|stopPropagation={() => toggleDropdown("sYear")}
                on:keydown|stopPropagation={(e) =>
                  handleTriggerKeydown(e, "sYear")}
              >
                <input
                  id="sYearInput"
                  type="text"
              ...
              {#if activeDropdown === "sYear"}<div
                ...
                >
                  {#each years as y}<button
                      class="ce-opt"
                      on:click|stopPropagation={() => selectOption("sYear", y)}
                      >{y}</button
                    >{/each}
                </div>{/if}
            </div>
        ...
        <div class="ce-input-group">
          ...
          {#if formData.eventType === "single_day"}
            <div class="ce-input-group">
              <input
                type="text"
                value={`${formData.eDay} ${translateMonth(formData.eMonth)} ${formData.eYear}`}
                class="ce-input-dis"
                disabled
          ...
          {:else}
            <div
              class="ce-date-row"
              class:error={validationErrors.has("endDate")}
            >
              <div class="ce-dd-wrap flex-1-5">
                <div
                  class="ce-trigger"
                  role="button"
                  tabindex="0"
                  on:click|stopPropagation={() => toggleDropdown("eDay")}
                  on:keydown|stopPropagation={(e) =>
                    handleTriggerKeydown(e, "eDay")}
                >
                  <input
                    id="eDayInput"
                    type="text"
                ...
                {#if activeDropdown === "eDay"}<div
                  ...
                  >
                    {#each days as d}<button
                        class="ce-opt"
                        on:click|stopPropagation={() => selectOption("eDay", d)}
                        >{d}</button
                      >{/each}
                  </div>{/if}
              </div>
              <div class="ce-dd-wrap flex-2">
                <div
                  ...
                  role="button"
                  tabindex="0"
                  on:click|stopPropagation={() => toggleDropdown("eMonth")}
                  on:keydown|stopPropagation={(e) =>
                    handleTriggerKeydown(e, "eMonth")}
                >
                  <input
                    id="eMonthInput"
                    type="text"
                    value={translateMonth(formData.eMonth)}
                    placeholder={lang.monthPlaceholder}
                    class="ce-input-dis"
                    readonly
                  /><span class="ce-arrow">▼</span>
                </div>
                {#if activeDropdown === "eMonth"}<div
                    class="ce-options"
                    role="listbox"
                    tabindex="0"
                    on:click|stopPropagation
              ...
              <div class="ce-dd-wrap flex-1-5">
                <div
                  ...
                  role="button"
                  tabindex="0"
                  on:click|stopPropagation={() => toggleDropdown("eYear")}
                  on:keydown|stopPropagation={(e) =>
                    handleTriggerKeydown(e, "eYear")}
                >
                  <input
                    id="eYearInput"
                    type="text"
          ...
          <div class="ce-input-group">
            ...
            <div class="ce-dd-wrap">
              <div
                ...
                role="button"
                tabindex="0"
                on:click|stopPropagation={() => toggleDropdown("startTime")}
                on:keydown|stopPropagation={(e) =>
                  handleTriggerKeydown(e, "startTime")}
              >
                <input
                  id="startTimeInput"
                  type="text"
                ...
                >
                  {#each times as t}
                    <button
                      class="ce-opt"
                      on:click|stopPropagation={() =>
                        selectOption("startTime", t)}>{t}</button
                    >
                  {/each}
                </div>
              {/if}
          ...
          <div class="ce-input-group">
            ...
            <div class="ce-dd-wrap">
              <div
                ...
                role="button"
                tabindex="0"
                on:click|stopPropagation={() => toggleDropdown("endTime")}
                on:keydown|stopPropagation={(e) =>
                  handleTriggerKeydown(e, "endTime")}
              >
                <input
                  id="endTimeInput"
                  type="text"
                ...
                >
                  {#each times as t}
                    <button
                      class="ce-opt"
                      on:click|stopPropagation={() =>
                        selectOption("endTime", t)}>{t}</button
                    >
                  {/each}
                </div>
              {/if}
      ...
      <div class="ce-card ce-config-card">
        ...
        <div class="ce-dual-row">
          <div class="ce-input-group">
            <label for="totalSlotsInput"
              >{lang.capacityLabel} <span class="ce-req">*</span></label
            ><input
              id="totalSlotsInput"
              type="number"
              bind:value={formData.totalSlots}
              placeholder="100"
              class="ce-input"
              class:error={validationErrors.has("totalSlots")}
              min="1"
            />
          </div>
          <div class="ce-input-group">
            <label for="distanceKmInput">{lang.distanceLabel}</label><input
              id="distanceKmInput"
              type="number"
              bind:value={formData.distanceKm}
              placeholder="5.0"
              class="ce-input"
              min="0"
              step="0.1"
      ...
      <div class="ce-card ce-config-card">
        ...
        <div class="ce-reward-explanation">
          ...
          <div class="ce-explanation-text">
            <strong>วิธีการแจกรางวัล:</strong>
            ระบบจะรวมผู้เข้าร่วมทั้งหมดมาเรียงลำดับตามจำนวนรอบที่วิ่งได้ ผู้ที่วิ่งได้มากกว่าจะได้รับรางวัลที่ดีกว่า
            โดยจำกัดรางวัลตามจำนวนที่กำหนด
        ...
        <div class="ce-input-group">
          ...
          <input
            id="totalRewardsInput"
            type="number"
            bind:value={formData.totalRewards}
            placeholder="300"
            class="ce-input"
            min="1"
          />
          <div class="ce-helper-text">
            จำนวนคนทั้งหมดที่จะได้รับรางวัล (เช่น 300 คนแรก)
          </div>
        </div>
          ...
          <div class="ce-reward-tier">
            <div class="ce-tier-header">
              ...
              {#if formData.rewardTiers.length > 1}
                <button
                  type="button"
                  class="ce-btn-remove-tier"
                  aria-label={`Remove reward tier ${idx + 1}`}
                  on:click={() => removeRewardTier(idx)}>×</button
                >
              {/if}
            </div>
            <div class="ce-dual-row">
              <div class="ce-input-group">
                ...
                <input
                  id={`reward-name-${idx}`}
                  type="text"
                  bind:value={tier.reward_name}
                  placeholder="เหรียญทอง, เสื้อยืด"
                  class="ce-input"
                />
              </div>
              <div class="ce-input-group">
                ...
                <input
                  id={`reward-req-${idx}`}
                  type="number"
                  bind:value={tier.required_completions}
                  placeholder="10"
                  class="ce-input"
                  min="1"
                />
            ...
            <div class="ce-tier-hint">
              💡 ผู้ที่วิ่งครบ <strong
                >{tier.required_completions || 0} รอบ</strong
              >
              และอยู่ใน <strong>{formData.totalRewards || 0} คนแรก</strong>
              จะได้รับ "{tier.reward_name || "รางวัล"}"
            </div>
          </div>
        {/each}
        <button type="button" class="ce-btn-add-tier" on:click={addRewardTier}
          >{lang.addTierBtn}</button
        >
      </div>
        ...
        <div class="ce-card ce-config-card">
          ...
          <div class="ce-toggle-group">
            <label class="ce-toggle-item"
              ><span>{lang.publicVisibility}</span><label
                class="ce-toggle-switch"
                ><input type="checkbox" bind:checked={formData.isPublic} /><span
                  class="ce-slider"
                ></span></label
              ></label
            >
            <label class="ce-toggle-item"
              ><span>{lang.activeOpen}</span><label class="ce-toggle-switch"
                ><input type="checkbox" bind:checked={formData.isActive} /><span
                  class="ce-slider"
                ></span></label
              ></label
            >
...
<style>
  ...
  .ce-explanation-text strong {
    color: var(--ce-primary);
  }
  .ce-explanation-text em {
    color: var(--ce-text-muted);
    font-style: italic;
  }
  .ce-tier-divider {
  ...
  .ce-tier-hint {
    ...
    border-left: 3px solid var(--ce-warning);
    border-radius: 6px;
    font-size: 0.85rem;
    color: var(--ce-text-muted);
    line-height: 1.5;
