    ...
    try {
      ...
      if (
        ...
        if (validTiers.length > 0) {
          // Ensure we've checked whether the reward-leaderboards API exists
          if (rewardApiAvailable === null) await checkRewardApiAvailability();

          // If the service is not available, skip creating/updating leaderboard configs
          if (!rewardApiAvailable) {
            console.info(
              "Reward API unavailable; skipping leaderboard config creation/update.",
            );
          } else {
              ...
              reward_tiers: validTiers.map((t, idx) => {
                ...
                return {
                  ...
                  required_completions: Number(
                    tierObj.required_completions || 0,
                  ),
                  min_rank: rankStart,
            ...
            try {
              ...
              if (editingEventId) {
                try {
                  ...
                  // ถ้า 404 คือไม่มีของเก่า (ปกติ) ไม่ต้องทำอะไร
                  if (checkErr.response?.status !== 404) {
                    console.warn(
                      "⚠️ Error checking existing config:",
                      checkErr,
                    );
                  }
                }
      ...
      <div class="ce-card ce-config-card">
        ...
        <div class="ce-reward-explanation">
          ...
          <div class="ce-explanation-text">
            ...
            ระบบจะรวมผู้เข้าร่วมทั้งหมดมาเรียงลำดับตามจำนวนรอบที่วิ่งได้
            ผู้ที่วิ่งได้มากกว่าจะได้รับรางวัลที่ดีกว่า
