<script lang="ts">
  ...
  const translations = {
    ...
    en: {
      ...
      confirmCalculateText:
        "System will fetch participation data and simulate ranking",
      confirmFinalize: "Finalize leaderboard?",
      confirmFinalizeText:
        "Once finalized, changes cannot be made. Rewards will be distributed.",
      ...
      step1Desc:
        "Calculate ranks and eligibility based on participation count (Simulate)",
      ...
      noConfigDesc:
        "This event has no leaderboard configuration. Please configure it in the event settings.",
  ...
  function applyFilters() {
    filteredRewards = rewards.filter((reward) => {
      ...
      if (selectedTier) {
        if (
          !reward.reward_tier ||
          reward.reward_tier.toString() !== selectedTier
        )
          return false;
      }
      return true;
    ...
    currentPage = 1;
    if (import.meta.env.DEV)
      console.debug("[rewards] applyFilters", {
        rewardsLength: rewards.length,
        filteredLength: filteredRewards.length,
        searchQuery,
        selectedStatus,
  ...
  async function resetLeaderboard() {
    ...
    Swal.fire({
      icon: "info",
      title: "Info",
      text: "Feature not available in this version",
    });
  }
  ...
  function exportAsCSV() {
    ...
    const rows = filteredRewards.map((r) => {
      ...
      const nisit =
        (user && (user.nisit_id || user.nisitId)) || r.user_nisit_id || "-";
    ...
    downloadFile(
      csv,
      `reward-leaderboard-${selectedEvent?.id}-${Date.now()}.csv`,
      "text/csv",
    );
  }
  ...
  function exportAsJSON() {
    ...
    downloadFile(
      json,
      `reward-leaderboard-${selectedEvent?.id}-${Date.now()}.json`,
      "application/json",
    );
  }
  ...
  async function loadRewardDefinitions(skip = 0, limit = 100) {
    try {
      ...
      if ((endpoints as any)?.rewards?.list)
        candidates.push((endpoints as any).rewards.list);
      ...
      for (const url of candidates) {
        try {
          ...
          if (import.meta.env.DEV)
            console.debug("[rewards] endpoint candidate failed", {
              url,
      ...
      rewardDefinitions = raw.map((r: any) => ({
        ...
        required_completions:
          typeof r.required_completions === "number"
            ? r.required_completions
            : (r.required_completions ?? 3),
        time_period_days:
          typeof r.time_period_days === "number"
            ? r.time_period_days
            : (r.time_period_days ?? 30),
        created_at: r.created_at,
        updated_at: r.updated_at,
      }));
      ...
      if (import.meta.env.DEV)
        console.log(
          "[rewards] loaded reward definitions from",
          usedUrl,
          "count=",
        ...
        rewardDefinitions = [
          {
            id: 1,
            name: "Early Bird",
            description: "Join early to get this badge",
            badge_image_url: "https://placehold.co/64x64",
          ...
          },
          {
            id: 2,
            name: "Consistent",
            description: "Attend regularly for a month",
            badge_image_url: "https://placehold.co/64x64",
  ...
  $: _endsAtRaw =
    (rewardConfig && rewardConfig.ends_at) ||
  ...
  $: endsAtFormatted = endsAt
    ? endsAt.toLocaleString(currentLang === "th" ? "th-TH" : "en-GB")
    : null;
</script>
          ...
          {#each paginatedEvents as event}
            <div class="event-card">
              <div class="event-image-wrapper">
                <img
                  ...
                  data-src={event.image ||
                    "https://placehold.co/400x200/1e293b/ffffff?text=No+Image"}
              ...
              <div class="event-card-body">
                ...
                <button
                  class="btn-view-rewards"
                  on:click={() => selectEventForRewards(event)}
                >
                  <svg
                    width="18"
                    height="18"
  ...
  {:else}
    <div class="leaderboard-view">
        ...
        <div class="header-info">
          ...
          <div class="event-meta-small">
            ...
            {#if rewardConfig}
              <span
                class="config-status"
                class:finalized={rewardConfig.is_finalized}
              >
        ...
        <div class="header-actions">
          ...
          {#if showExportMenu}
            <div class="export-menu">
              <button
                class="export-option"
                on:click={() => exportLeaderboard("csv")}
              >
                <svg
                  width="16"
                  height="16"
              ...
              <button
                class="export-option"
                on:click={() => exportLeaderboard("json")}
              >
                <svg
                  width="16"
                  height="16"
      ...
      {#if !rewardConfig && !loading}
        <div class="empty-state">
          <p style="color: #ef4444; font-size: 1.2rem; font-weight: bold;">
            {lang.noConfigTitle}
          </p>
              ...
              <div class="filter-menu">
                <button
                  ...
                  on:click|stopPropagation={() => {
                    ...
                    statusDropdownOpen = false;
                    applyFilters();
                  }}>{lang.allStatus}</button
                >
                <button
                  class="filter-option"
                  class:selected={selectedStatus === lang.rewarded}
                  on:click|stopPropagation={() => {
                    selectedStatus = lang.rewarded;
                    statusDropdownOpen = false;
                    applyFilters();
                  }}
                ...
                <button
                  class="filter-option"
                  class:selected={selectedStatus === lang.qualified}
                  on:click|stopPropagation={() => {
                    selectedStatus = lang.qualified;
                    statusDropdownOpen = false;
                    applyFilters();
                  }}
                ...
                <button
                  class="filter-option"
                  class:selected={selectedStatus === lang.notQualified}
                  on:click|stopPropagation={() => {
                    selectedStatus = lang.notQualified;
                    statusDropdownOpen = false;
                    applyFilters();
                  }}
