<script lang="ts">
  ...
  async function selectEventForLogs(event: Event) {
    // 🚀 UX Optimization: Switch view IMMEDIATELY
    selectedEvent = event;
    eventDates = [];
    logs = []; // Clear previous logs
    view = "logs"; // Switch screen instantly
    loading = true; // Show local spinner in table
    ...
    // Note: performAutoRefresh handles loading=false in its own flow if it calls loadSnapshotEntries
    // But if we have snapshots:
    else if (snapshots.length > 0) {
      currentSnapshotId = snapshots[0].snapshot_id;
      // loadSnapshotEntries handles loading=true/false internally
      await loadSnapshotEntries(event.id, currentSnapshotId);
    } else {
      // No snapshots, stop loading
      loading = false;
    }
  }
