<script lang="ts">
      ...
      const newEvents = apiData
        ...
        .map((item: any) => {
          ...
          return {
            ...
            is_full:
              item.is_full || item.participant_count >= item.max_participants,
            ...
            // [NEW] Single Day / Multi Day fields
            event_type:
              item.event_type === "multi_day" ? "multi_day" : "single_day",
  ...
  async function fetchUniqueParticipantCounts() {
    ...
    for (let i = 0; i < eventsCopy.length; i += BATCH_SIZE) {
      ...
      // For officer, use participant_count from event directly
      batch.forEach((event) => {
  ...
  async function updateUserStatus() {
    try {
      ...
      const res = await fetch(
        `${BASE_URL}/api/participations/user/${userId}?_=${cacheBuster}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        events = events.map((e) => {
          ...
          for (const record of myRecords) {
            ...
            if (s === "CANCELLED" || s === "CANCEL") {
              ...
              console.log(
                `[Event ${e.id}] Skipping CANCELLED record:`,
                record.id,
              );
              continue;
            }
          ...
          if (e.event_type === "multi_day" && myRecords.length > 0) {
            ...
            console.log(
              `[Event ${e.id}] Checking isJoinedToday - Today (server): ${todayStr}`,
            );
            ...
            isJoinedToday = myRecords.some((record: any) => {
              ...
              if (recordDate) {
                const recordDateOnly = new Date(recordDate)
                  ...
                  .split("T")[0];
                console.log(
                  `  - Record ${record.id}: date=${recordDateOnly}, status=${s}, match=${recordDateOnly === todayStr}`,
                );
          ...
          if (finalRecord) {
            console.log(
              `[Event ${e.id}] Active record found:`,
              finalRecord.id,
              finalRecord.status,
              `isJoinedToday: ${isJoinedToday}`,
            );
            if (finalRecord.created_at) {
              console.log(
                `  - Record date: ${new Date(finalRecord.created_at).toISOString()}`,
              );
            }
          } else {
          ...
          return {
            ...
            participationStatus: finalRecord
              ? finalRecord.status.toUpperCase()
              : null,
            participationDate: finalRecord
              ? finalRecord.created_at || finalRecord.date
              : null,
            isUpcoming: e.startDate ? new Date(e.startDate) > now : false,
            canRegisterToday:
              e.is_active && e.is_published && !isJoined && !e.is_full,
  ...
  async function handleRegister(eventItem: EventItem) {
    ...
    if (isMultiDay) {
      ...
      if (
        eventItem.max_checkins_per_user &&
        ...
        if (checkinCount >= eventItem.max_checkins_per_user) {
          Swal.fire({
            icon: "info",
            title:
              lang === "th" ? "ลงทะเบียนครบแล้ว" : "Registration Limit Reached",
            text:
              lang === "th"
    ...
    if (eventItem.endDate) {
      ...
      if (now > endDate) {
        Swal.fire({
          ...
          text:
            lang === "th" ? "กิจกรรมนี้หมดเวลาแล้ว" : "This event has ended",
          confirmButtonText: "OK",
        });
        return;
      }
    ...
    const result = await Swal.fire({
      title: isMultiDay
        ...
        : lang === "th"
          ? "ยืนยันการสมัครวันนี้?"
          : "Confirm Daily Registration?",
      text: isMultiDay
        ? lang === "th"
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (isMultiDay) {
          ...
          await performJoin(
            `${BASE_URL}/api/participations/join`,
            eventItem,
            token,
          );
        }
  ...
  async function performPreRegister(eventItem: EventItem, token: string) {
    const preRegRes = await fetch(
      `${BASE_URL}/api/participations/pre-register/${eventItem.id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  async function performJoin(
    ...
    if (joinRes.ok) {
      ...
      const wasReactivated =
        responseData.id && eventItem.participationId === responseData.id;
  ...
  function toggleDetails(index: number) {
    ...
    events = events.map((e) => ({
      ...e,
      isExpanded: e.id === targetId ? !e.isExpanded : false,
    }));
  }
    ...
    <div class="header-inner">
        ...
        <nav class="nav-menu desktop-only">
          {#each menuItems as item}
            <button
              ...
              onclick={() => selectView(item.id, item.path)}
            >
              <svg
                class="nav-icon"
                fill="none"
      ...
      <div class="user-zone">
        ...
        <div class="lang-switch desktop-only">
          <button class:active={lang === "th"} onclick={() => setLang("th")}
            >TH</button
          >
          <span class="sep">|</span>
          <button class:active={lang === "en"} onclick={() => setLang("en")}
            >EN</button
          >
        </div>
  ...
  {#if isMobileMenuOpen}
    ...
    <div class="mobile-drawer" transition:slide={{ axis: "x", duration: 300 }}>
      <div class="drawer-header">
        <span class="brand-name" style="font-size: 1.4rem;">MENU</span><button
          class="close-btn"
          onclick={() => (isMobileMenuOpen = false)}>&times;</button
        >
      </div>
      ...
      <button class="drawer-item logout-special" onclick={handleLogout}
        >Logout</button
      >
    </div>
  {/if}
              ...
              <div class="card-content">
                <div class="card-header-row">
                  ...
                  <div class="badges-col">
                    {#if !event.is_published}
                      <span class="status-badge no-active">
                        {#if lang === "th"}
                          ...
                          ></span>
                        {:else}
                          NOT OPEN <span style="font-size: 0.8em; opacity: 0.8;"
                          ></span>
                        {/if}
                      </span>
                    {:else if !event.is_active}
                      <span class="status-badge no-active"
                        >{t[lang].status_not_open}</span
                      >
                    {:else if event.participationStatus === "CHECKED_IN"}
                      <span class="status-badge pending"
                        >{t[lang].status_pending}</span
                      >
                    {:else}
                    ...
                    <div class="type-badges">
                      <span
                        class="type-badge"
                        class:multi={event.event_type === "multi_day"}
                      >
                        {event.event_type === "multi_day"
                          ? t[lang].event_type_multi
                          : t[lang].event_type_single}
                      </span>
                      {#if event.event_type === "multi_day" && event.allow_daily_checkin}
                        <span class="type-badge daily"
                          >{t[lang].daily_checkin}</span
                        >
                      {/if}
                    </div>
                  </div>
                    ...
                    <div class="info-pills">
                      ...
                      {#if event.event_type === "multi_day" && event.allow_daily_checkin}
                        <div class="info-pill progress-pill">
                          ...
                          <span>
                            {t[lang].checkin_progress}: {event.checkin_count}
                            {lang === "th" ? "ครั้ง" : "times"}
                          </span>
                        </div>
                      {/if}
                ...
                <div class="card-footer-actions">
                  ...
                  <div class="footer-actions">
                    {#if event.isJoined}
                      <button
                        class="register-btn running"
                        onclick={() => navigateToMyEvents("officer")}
                      >
                    ...
                    {:else if !event.is_active}
                      <button class="register-btn no-active" disabled
                        >{t[lang].status_not_open}</button
                      >
                    {:else if event.is_full}
                      <button class="register-btn full" disabled
                        >{t[lang].status_full}</button
                      >
...
<style>
  ...
  .event-card:hover {
    ...
    box-shadow:
      0 24px 36px -6px rgba(0, 0, 0, 0.5),
  ...
  .register-btn:hover {
    ...
    box-shadow:
      0 12px 28px -4px rgba(16, 185, 129, 0.65),
